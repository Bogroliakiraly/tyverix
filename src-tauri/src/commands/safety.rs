...
#[tauri::command]
pub async fn create_restore_point(description: String) -> AppResult<String> {
    blocking(move || {
        // Checkpoint-Computer needs admin + System Restore enabled. By default
        // Windows throttles to one point per 24h; we clear that limit for this
        // call so a pre-operation checkpoint is reliable.
        let script = format!(
            r#"
...
#[tauri::command]
pub async fn backup_registry() -> AppResult<String> {
    blocking(move || {
...
#[tauri::command]
pub async fn list_action_log(app: tauri::AppHandle) -> AppResult<Vec<ActionRecord>> {
    blocking(move || {
...
#[tauri::command]
pub async fn undo_action(app: tauri::AppHandle, id: String) -> AppResult<()> {
    blocking(move || {
        let state = app.state::<AppState>();
        let rec = {
            let log = state
