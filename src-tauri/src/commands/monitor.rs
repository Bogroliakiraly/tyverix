...
#[tauri::command]
pub fn get_system_snapshot(state: State<AppState>) -> AppResult<SystemSnapshot> {
    let mut sys = state
        .sys
        .lock()
...
#[tauri::command]
pub fn list_processes(state: State<AppState>) -> AppResult<Vec<ProcessInfo>> {
    let mut sys = state
        .sys
        .lock()
        .map_err(|_| AppError::other("system state is locked"))?;
...
#[tauri::command]
pub fn kill_process(state: State<AppState>, pid: u32) -> AppResult<()> {
    let sys = state
        .sys
        .lock()
        .map_err(|_| AppError::other("system state is locked"))?;
